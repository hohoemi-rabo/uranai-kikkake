...
- [×] `hooks/useAuth.tsx` を作成(`{ session, isAuthenticated, isLoading, signIn, signOut }`。フラット配置)
...
- [×] API リクエストの 401 で自動ログアウトするユーティリティを `lib/api.ts` に置く(雛形のみ。エンドポイント接続は 09 で実施)
...
- ログアウト UI は本チケットでは「ホーム画面下部の動作確認用ボタン」を仮置きしている。チケット 20(設定画面)で正規の場所に移管する
- 401 ハンドラはモジュールスコープのシングルトン(`registerOn401`)。`AuthProvider` 起動時に `signOut` を登録するパターンで、`lib/` 側から `hooks/` を import せずに済む(循環防止)
- ファイル配置はフラット(`hooks/useAuth.tsx`・`lib/api.ts`・`lib/auth/secureStore.ts`)。REQUIREMENTS.md §9 の `src/` 案は採用しない
