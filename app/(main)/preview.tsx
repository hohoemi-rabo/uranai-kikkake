  ...
  if (!uri) {
    return (
      <ScreenBackground style={{ alignItems: 'center', justifyContent: 'center' }}>
        <Text className="text-base font-rounded text-slate-200">画像が見つかりません</Text>
        <Pressable onPress={() => router.back()} className="mt-4 p-3">
          <Text className="text-base font-rounded-bold text-slate-100 underline">戻る</Text>
        </Pressable>
      </ScreenBackground>
    );
  }
  ...
  return (
    <ScreenBackground edges={['top', 'bottom']}>
      <View className="flex-1 px-6 pt-2 pb-6">
        ...
        >
          <Text className="text-base font-rounded-bold text-slate-200">← 撮り直す</Text>
        </Pressable>
