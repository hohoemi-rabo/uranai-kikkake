...
- **`useAuthRequest` は React Hook、関数 API には包めない**: `signIn.ts` の関数チェーンとは別ルートで login 画面が直接呼ぶ。`useAuth().signInWithSession(session)` を追加して、任意のセッション(Google 取得)で認証を完了するインターフェイスを切り出す
- **Hooks rule 準拠のための分離コンポーネント**: `GoogleLoginButton` / `StubLoginButton` をサブコンポーネント化して、AUTH_MODE で切り替える。条件付き Hook 呼び出しを避けるためにこの分離が必須(`useGoogleSignIn` を `if (AUTH_MODE === 'google')` の中で呼ぶのは違反)
- **id_token の sub 取得は自前 base64url decode で十分**(`lib/auth/google.ts:decodeJwtPayload`)。jose を入れる必要なし(jwt は base64url JSON、ペイロード 1 行で取れる)。padding は `(4 - len % 4) % 4` で計算
- **`WebBrowser.maybeCompleteAuthSession()` をモジュールトップで呼ぶ**: redirect 後のアプリ復帰時にブラウザを閉じる必須処理。useEffect 内ではなく **モジュール初期化時に 1 回呼ぶ** が公式パターン
- **`Google.useAuthRequest` の Client ID プロパティは undefined OK**: 空文字を `|| undefined` で undefined に変換すると、iOS / Android で未対応プラットフォーム時の警告が消える。stub モードで Hook を空 ID で呼んでもエラーにならない
- **Android パッケージ名は dev/prod の 2 種類**: `jp.hohoemi-rabo.uranaikikkake` / `.dev`。Google Cloud Console で **2 つの Android Client ID** を作って SHA-1 をそれぞれ登録するのが運用上分かりやすい。`EXPO_PUBLIC_GOOGLE_ANDROID_CLIENT_ID` には 1 つだけ書けば OK(`useAuthRequest` がパッケージ名 + SHA-1 で自動マッチング)
- **Web Client ID が id_token の audience**: Workers の `GOOGLE_CLIENT_IDS` には **Web Client ID** を入れる(Android Client ID ではない)。これが `id_token.aud` に乗ってくる
- **Client ID は機密ではない**(audience 識別子)。`EXPO_PUBLIC_*` で JS バンドルに展開しても OK。Client Secret は Public Client(モバイル)では使わない
- **`EXPO_PUBLIC_AUTH_MODE=stub` で Expo Go を温存**: ローカル `.env` を stub のまま、`eas.json` の preview/production だけ google にして使い分け。本チケット完了後も Expo Go 開発フローは死なない
- **`expo-auth-session/providers/google` は Config Plugin 不要**: `app.config.ts` の `plugins` に追加しなくて良い。`scheme: 'uranaikikkake'` がすでにあれば動く(`prebuild --clean` でネイティブ層がスキーマを拾う)
- **`signIn.ts` の `google` ブランチは案内エラーを残す**: 直接呼ばれないが、誤って `signIn('google')` を呼んだ時に「`useGoogleSignIn` 経由で」と教える役
