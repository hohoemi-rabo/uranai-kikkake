...
NativeWind の `text-7xl` 以上(`font-size: 4.5rem` 以上 / `line-height: 1`)で **絵文字を出すと頭が切れる**。`text-7xl` のような大きいサイズで絵文字を 1 文字だけ出す場合は `<Text style={{ lineHeight: 96 }}>` のように font-size の 1.3 倍ほどを明示する。例: `app/(auth)/login.tsx` / `app/+not-found.tsx` の 🔮。`text-6xl` 以下なら大半セーフだが、🔮 / 🎉 / 🌟 のような縦長グリフは念のため明示推奨。
...
**「顔相」(UI ラベル)と「魅力発見モード」(コンセプト名)の意図的な不一致**: UI 表示(TabSwitcher / 結果バッジ / ResultCard の MODE_LABEL)は **「顔相」**(3 タブで文字数 2 揃え)、`workers/src/prompts.ts` の AI ペルソナ「AI魅力発見士」・`REQUIREMENTS.md` §6.2.1・`docs/store/play-listing-draft.md` などのコンセプト/マーケコピーは **「魅力発見モード」のまま**。**統一を狙って勝手にリネームしない**(2026-05 にユーザーが意図的に分離した: UI はバランスのため 2 文字、ブランドは「魅力発見」のポジティブさを残す)。grep で「魅力発見」が出てきても UI 側だけはそのまま据え置き。
...
- **SecureStore に新キーを追加するときは optional 必須**: `name`/`email`/`pictureUrl`(プロフィール用)を `AuthSession` 型に追加した際、**必須にすると既存ユーザーが loadSession 時に弾かれて強制ログアウト**になる。`?` 付きの optional にすれば既存セッションは維持され、**新キーは次回ログイン以降にしか populate されない**(既存ユーザーは「ログアウト → 再ログイン」で取得される)。新フィールド追加時はリリースノートに「再ログインで反映」を書く運用。`saveSession` で空値のキーは明示 `deleteItemAsync` する設計にしてあるので、provider 切替/別 Google アカウント切替時に古いプロフィールが残らない(`lib/auth/secureStore.ts` 参照)。
- **`UserAvatar` のフォールバック 3 パターン**: Google + 画像取得成功 / Google + 画像取得失敗(`onError` で `setImageFailed`)→ イニシャル円 / stub → 「開」イニシャル。**画像 URL があれば必ず読める保証は無い**(Google 側 CDN 障害・端末ネットワーク不通)ので `<Image onError>` 経由のフォールバックは必須。`expo-image` のキャッシュ(`memory-disk`)併用で 2 回目以降はオフラインでも出る
...
- **採用した解決策: Web Client ID + PKCE + HTTPS リダイレクトプロキシ + Workers 側トークン交換**。`expo-web-browser` の `openAuthSessionAsync` で Custom Tabs を開き、HTTPS の redirect_uri(GitHub Pages の HTML)経由で custom scheme に戻す
- **トークン交換はアプリではなく Workers でやる(重要・最初の設計ミス)**: 「ウェブアプリケーション」型 OAuth クライアントは confidential client で、Google がトークン交換に `client_secret` を**必須**にする(失敗時 `invalid_request` / `client_secret is missing`)。PKCE だけで済むのは Android/iOS/デスクトップ型(public client)のみ。`client_secret` は機密なのでアプリバイナリに埋めず Workers の secret に置き、アプリは `code`/`code_verifier`/`clientId` を `POST /api/auth/google` に送るだけにする(workers/CLAUDE.md 参照)

### 実装の要点

- **`lib/auth/google.ts` は 2 関数に分割**: `startGoogleSignIn()`(OAuth セッションを開くだけ)と `completeGoogleSignIn(params)`(callback 画面から呼ぶ、トークン交換)。Hook の制約がないので login 画面の Hook 分離は不要、`login.tsx` は `GoogleLoginButton`/`StubLoginButton` の AUTH_MODE 分岐だけ
- **deep link を受ける専用画面 `app/oauth/google/callback.tsx` が必須**: `uranaikikkake://oauth/google/callback` は expo-router のルートが無いと "Unmatched Route" になる。この画面が `completeGoogleSignIn` → `signInWithSession` → `router.replace('/(main)')` を担う
...
- **Client ID は機密ではない**(audience 識別子)。`EXPO_PUBLIC_*` で JS バンドルに展開しても OK。一方 **Web Client の `client_secret` は機密**なので絶対にアプリに入れない(Workers の secret のみ)
...
### 実機デバッグで判明した罠(callback 二重マウント・PKCE 永続化)

- **callback 画面は端末によって二重マウントされる**: `uranaikikkake://...` の deep link は (a) OS が配信 → expo-router、と (b) `openAuthSessionAsync` が `{type:'success'}` を返す → `startGoogleSignIn` 側で `router.replace`、の **両方が発火する端末がある**。両方が `completeGoogleSignIn` を呼ぶと、認可コードは 1 回限り有効なので 2 回目のトークン交換が `invalid_grant` で失敗する(ログイン自体は成功するのに余計なエラーダイアログが出る)
- **二重実行ガードはモジュールレベルの Promise キャッシュ**: `lib/auth/google.ts` の `inFlightCompletion`。`completeGoogleSignIn` を **非 async** にして「`if (inFlightCompletion) return it` → `inFlightCompletion = run(...)`」を同期的=アトミックに実行。`startGoogleSignIn` で `null` リセット(再ログイン対応)。callback 画面ごとの `useRef` ガードは **インスタンス単位なので二重マウントには効かない**
- **PKCE 検証値は SecureStore に永続化**(module 変数 NG): OAuth 中(Custom Tabs 表示中)に Android が低メモリでアプリを kill すると module 変数は消え、callback で `no-pending` になる。`completeGoogleSignIn` は読み出し直後に削除して 1 回消費にする
- **`openAuthSessionAsync` の戻り値 `success` も処理する**: 大半の端末は OS が deep link を配信する(`result.type === 'dismiss'`)が、Custom Tabs がリダイレクトを横取りする端末では `success` が返り OS 配信が起きない。`success` のときは `router.replace('/oauth/google/callback', params)` で callback 画面に流して経路を統一(上の二重実行ガードがあるので両経路発火しても安全)
- **オンボーディング完了フラグはリアクティブな Context にする**(one-shot 読みは NG): `app/_layout.tsx` のルート `<Redirect href="/(auth)/onboarding">` が `onboardingDone` を見るが、`useEffect` で 1 回読むだけだと、同一セッションでオンボ完了 → ログイン成功後の `router.replace('/(main)')` が **stale な `onboardingDone=false` に弾かれてオンボに戻る**。`hooks/useOnboarding.tsx` の Context にして完了時に state も即更新する
...
- **2026-05 リデザイン以降は SVG-only パイプライン**: ダーク化に伴い `assets/svg/icon.svg`(濃紺グラデ + ゴールド「占」+ スパーク)を主軸に再設計。**`assets/source/icon.png`(旧 master PNG)はもう使われていない** — `scripts/generate-icons.mjs` は 5 つの SVG ソースだけから 9 つの PNG を生成する。今後デザイン差し替えは **SVG を編集 → `npm run generate-icons`** で完結する。`source/` ディレクトリは将来の代替案保存用に残してあるが、参照されないので削除しても挙動は変わらない
- **アイコン変更は OTA で配信できない**: `icon.png` / `splash-icon.png` / `android-icon-*.png` は **ネイティブビルド時に AndroidManifest / Info.plist 経由で焼き込まれる**ため、`eas update` では反映されない。**必ず `eas build` を踏む**。SVG 編集 → `generate-icons` → `eas build --profile <name>` の順
- **`adaptiveIcon.backgroundColor` は `app.config.ts` で別管理**: foreground PNG が透過の場合、四隅・マスクの外側を埋める色は **PNG ではなく config 値**で決まる。テーマ変更時に PNG だけ更新して config の値を忘れると、Android 端末で四隅から旧色が見える事故になる(2026-05 のダーク化で対応済み: `#1E1B4B`)
